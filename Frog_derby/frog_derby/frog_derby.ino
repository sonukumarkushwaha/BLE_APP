...
//unsigned long single_step = 10;  // adjust timing for your step pulse (µs or ms)
...
void Task1(void *pvParameters) {
  ...
  while (true) {
    ...
    delayMicroseconds(270);           // Step delay
...
void onGameMessage(struct_message data) {
  ...
  if (data.gameStatus == 100) {
    if (data.id == 1)incoming1 = 500;
    if (data.id == 2)incoming2 = 500;
    if (data.id == 3)incoming3 = 1;
    if (data.id == 4)incoming4 = 1;
    if (data.id == 5)incoming5 = 1;
    data.gameStatus = 0;
    datapass = true;
  }
}
...
// <<=== RESET ADDED ===
  ...
  incoming1 = incoming2 = incoming3 = incoming4 = incoming5 = 0;

  previousMillis1 = previousMillis2 = previousMillis3 = previousMillis4 = previousMillis5 = 0;
  ...
  while (1) {
    if (passlimit1 == 1 && passlimit2 == 1 && passlimit3 == 1 && passlimit4 == 1 && passlimit5 == 1) {
      ...
      digitalWrite(Dir, LOW);               // Step reverse coming to start point
      startpoint();
    }
  }
  ...
  while (true) {
    if (digitalRead(start_button) == 0) {
      if (passlimit1 == 1 && passlimit2 == 1 && passlimit3 == 1 && passlimit4 == 1 && passlimit5 == 1) {
        digitalWrite(Dir, HIGH);  // Step forward toward gameplay
        passlimit1 = 0;
        passlimit2 = 0;
        passlimit3 = 0;
        passlimit4 = 0;
...
void loop() {
  ...
  if (pass5 >= 1) {
    ...
    if (currentMillis5 - previousMillis5 >= pass5) {
